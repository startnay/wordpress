...
class SPClass {
    ...
    function __construct($post_id = NULL) {
        if (!empty($post_id) && is_array($post_id)) {
            ...
            return $this->addClass(
                            $values['name'], $values['description'], $values['department'], $values['semester'], $values['enrollment']
            );
    ...
    function getGroup($group_id = NULL) {
        ...
        if (!empty($this->group)) {
            ...
            if (is_array($this->forum_id))
                $this->forum_id = $this->forum_id[0];
